...
type Row = { roundId: string; status: string; claimable: number; claimed: number; expected: number };

const rowFor = (agentId: string, r: RoundReturns): Row | null => {
  const me = r.investors.find((i) => i.agentId === agentId);
  return me ? { roundId: r.roundId, status: r.status, claimable: me.claimableUsdc, claimed: me.claimedUsdc, expected: me.expectedUsdc } : null;
};
...
export function AgentReturns({ agentId, roundIds, roundNames }: AgentReturnsProps) {
  ...
  return (
    <Panel eyebrow="revenue share" title="Claimable now" tone="accent" action={<span className="num text-[10.5px] text-accent">{num(claimable)} USDC</span>} bodyClassName="p-0">
      {rows === null ? (
        <p className="p-4 font-mono text-[11px] text-dim"><span className="live-dot">●</span> reading escrows…</p>
      ) : rows.length === 0 ? (
        <p className="p-4 text-[12px] text-muted">No on-chain positions yet — returns appear once this agent has invested in an escrowed round.</p>
      ) : (
          ...
          {rows.map((row) => (
            <li key={row.roundId} className="flex flex-wrap items-center justify-between gap-2 px-4 py-2 font-mono text-[11px]">
              <span className="min-w-0">
                ...
                <span className="ml-1.5 text-dim">{row.status} · {num(row.claimed)}/{num(row.expected)} claimed</span>
              </span>

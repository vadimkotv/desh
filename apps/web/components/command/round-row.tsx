...
type RoundRowProps = { round: RoundDetail; preview: DueDiligencePreview | null };

// Compact open-round card for the command center's left rail.
export function RoundRow({ round, preview }: RoundRowProps) {
    ...
    <article className="group flex flex-col gap-2.5 rounded-lg border border-line bg-panel/90 p-3 transition-colors hover:border-accent/40">
      <div className="flex items-start justify-between gap-2">
        <div className="min-w-0">
          <Link href={`/rounds/${round.id}`} className="block truncate text-[13px] font-semibold text-bright group-hover:text-accent">
            {round.startup.name}
          </Link>
        ...
        {preview ? (
          ...
          <span className="flex h-10 w-10 items-center justify-center rounded-full border border-dashed border-line-strong font-mono text-[9px] text-dim" title="no DD report yet">
            n/a
          </span>
        )}
      </div>
